...
createCard = function(cardId, locId, cardInfo){
	...
	if(cardInfo ==="")
		cardInfo = "back"
	newCard.bind('dblclick', function(event) {check($(this), $('#old-local-info').data('old-local-info'));})
...
$(document).ready(function() {
	...
	//shift player names into correct places
	$("[data-playerId=2]").css('top', getPixelLoc(2,0,swappedLocIds)[1]-40).css('left', getPixelLoc(2,0,swappedLocIds)[0]).rotate(0);
	$("[data-playerId=3]").css('top', getPixelLoc(3,0,swappedLocIds)[1]+60).css('left', getPixelLoc(3,0,swappedLocIds)[0]+150).rotate(90);
	$("[data-playerId=4]").css('top', getPixelLoc(4,0,[0,1,2,3,4,5])[1]+180).css('left', getPixelLoc(4,0,[0,1,2,3,4,5])[0]+25).rotate(0); //rotate 0 so player 4's name faces rightside up
	$("[data-playerId=5]").css('top', getPixelLoc(5,0,[0,1,2,3,4,5])[1]+80).css('left',getPixelLoc(5,0,[0,1,2,3,4,5])[0]-95).rotate(270);
	$("#score").css('left',getPixelLoc(1,0,swappedLocIds)[0]+150).css('top',getPixelLoc(1,0,swappedLocIds)[1]+70) //setup score's location
	$("[id=flag]").css('top', getPixelLoc(2,0,swappedLocIds)[1]-30).css('left', getPixelLoc(2,0,swappedLocIds)[0]+100).rotate(0);
